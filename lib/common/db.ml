...
let insert_user (u : user) : user Lwt.t =
  ...
  Pg.query_row c
    {|INSERT INTO users (email, created_at, updated_at, account_id)
      VALUES ($1, $2, $3, $4)
      RETURNING *|}
    ~p:(unpack_user u [ `email; `created_at; `updated_at; `account_id ])
  >|= pack_user
...
VALUES ($1, $2, $3, $4, $5)
RETURNING *|}
    ~p:(unpack_status s [ `uri; `text; `created_at; `updated_at; `account_id ])
  >|= pack_status

let update_status_uri (s : status) =
  do_query @@ fun c ->
  Pg.query_row c "UPDATE statuses SET uri = $1 WHERE id = $2 RETURNING *"
    ~p:(unpack_status s [ `uri; `id ])
  >|= pack_status
...
RETURNING *|}
    ~p:
      (unpack_follow f
         [ `created_at; `updated_at; `account_id; `target_account_id; `uri ])
  >|= pack_follow
