...
type accept = { id : string; actor : Yojson.Safe.t; obj : t }
...
and follow = { id : string; actor : string; obj : string }
...
and undo = { id : string; actor : Yojson.Safe.t; obj : t }
...
(* FIXME: The following to_*, of_*, make_* functions should be generated by PPX *)
let to_accept = function Accept r -> Some r | _ -> None
let to_create = function Create r -> Some r | _ -> None
let to_follow = function Follow r -> Some r | _ -> None
let to_note = function Note r -> Some r | _ -> None
let to_person = function Person r -> Some r | _ -> None
let to_undo = function Undo r -> Some r | _ -> None
let of_accept r = Accept r
let of_create r = Create r
let of_follow r = Follow r
let of_note r = Note r
let of_person r = Person r
let of_undo r = Undo r
let make_follow ~id ~actor ~obj : follow = { id; actor; obj }
let make_accept ~id ~actor ~obj : accept = { id; actor; obj }
let make_undo ~id ~actor ~obj : undo = { id; actor; obj }

let make_create ~id ~actor ~published ~to_ ~cc ~obj : create =
...
let rec of_yojson (src : Yojson.Safe.t) =
  ...
  | "Accept" ->
      ...
      |> of_accept
  | "Follow" ->
      make_follow ~id ~actor:(string Actor) ~obj:(string Object) |> of_follow
  | "Undo" ->
      make_undo ~id ~actor:(get Actor) ~obj:(get Object |> of_yojson) |> of_undo
  | "Create" ->
      ...
      make_create ~id ~actor:(get Actor) ~published ~to_ ~cc ~obj |> of_create
  | "Note" ->
      let published = string Published in
      let to_ = list To |> List.map expect_string in
      let cc = list Cc |> List.map expect_string in
...
let rec account_of_person' (r : person) : Db.Account.t Lwt.t =
...
and account_of_person (r : person) : Db.Account.t Lwt.t =
...
and fetch_account ?(scheme = "https") by =
  let make_new_account (uri : string) =
    fetch_activity ~uri >|= of_yojson >|= to_person >|= Option.get
    >>= account_of_person'
  in
  match by with
...
let note_of_status (s : Db.Status.t) : note Lwt.t =
...
let rec status_of_note' (note : note) : Db.Status.t Lwt.t =
...
and status_of_note (note : note) : Db.Status.t Lwt.t =
...
and fetch_status ~uri =
  ...
  | s -> Lwt.return s
  | exception Sql.NoRowFound ->
      fetch_activity ~uri >|= of_yojson >|= to_note >|= Option.get
      >>= status_of_note' >>= Db.Status.save_one

let create_note_of_status (s : Db.Status.t) : create Lwt.t =
  let%lwt self = Db.Account.get_one ~id:s.account_id () in
