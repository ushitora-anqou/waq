        ...
        with Sqlx.Error.NoRowFound -> (
          ...
          | Some x ->
              ...
              Model.PreviewCard.make ~url:x.url ~title:x.title
                ...
                ~html:x.html ~author_name:x.author_name ~author_url:x.author_url
                ~provider_name:x.provider_name ~provider_url:x.provider_url
                ~width:x.width ~height:x.height ~embed_url:x.embed_url
                ?blurhash:x.blurhash ()
              |> Option.some))
